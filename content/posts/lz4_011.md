...
The tests below are only from the lz4_flex versions that contain `unsafe`.
...
There's no simple answer to that, but I can provide details of the performance improvements of v0.11.
...
Adds `auto_finish` to `FrameEncoder`, which flushes on drop. 
