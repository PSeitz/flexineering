...
An example where bounds checks hoisting was unable to be performed is a backwards [loop](https://github.com/PSeitz/lz4_flex/blob/main/src/sink.rs#L191-L195), resulting in two bounds
...
```
[#99](https://github.com/PSeitz/lz4_flex/pull/99) (thanks @CosmicHorrorDev).
### Handle empty input
Empty input was ignored previously and didn't write anything. Now an empty Frame is written. This improves compatibility with the reference implementation and some corner cases.
...
`default-features=false` caused the API to be unsound. With `unchecked-decode` it requires deliberate action to enable the unsafe API.
